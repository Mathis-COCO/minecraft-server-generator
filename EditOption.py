...
def Modification1P(opt1, user_number):
    while True :
        try:
...
def Modification2P(opt1, opt2, user_number):
    while True :
        try:
            parameter_chosen = int(input("veuillez choisir une valeur : "))
        except ValueError :
...
with open("./server.properties","r") as options :
        ...
        if i == 50:
            ...
            if user_number in num_list:
                if 'percentage' in option_no_param[0] :
                    new_parameter = Modification2P(0,100, user_number)
                if user_number == 13 :
                    new_parameter = Modification2P(0,3, user_number)
                if (user_number == 32) or (user_number == 39) or (user_number == 35) :
                    new_parameter = Modification2P(0,65535, user_number)
                if (user_number == 21) or (user_number == 26):
                    new_parameter = Modification2P(0,256, user_number)
                if (user_number == 22) or (user_number == 23) or (user_number == 24) :
                    new_parameter = Modification1P(0, user_number)
                if user_number == 28:
                    new_parameter = Modification2P(0,4, user_number)
                if (user_number == 29) or (user_number == 33) or (user_number == 44) :
                    new_parameter = Modification1P(-1, user_number)
                if user_number == 48 :
                    new_parameter = Modification1P(0, user_number)
                if user_number == 25 :
            ...
            while True :
                try:
                    ...
                    new_parameter = option_no_param[0]+"="+parameter_chosen
                    break
    options = open("./server.properties","w+")
